  ...
  export default function SetupAccountPage() {
    ...
    // 1. Fetch User Data EARLY (so it's ready for Step 3)
    useEffect(() => {
      async function fetchData() {
        ...
        if (user) {
          ...
          // Pre-fill form immediately if we have data
          profileForm.reset({
            name: user.name || '',
    ...
    const handleVerify = async (data) => {
      ...
      try {
        ...
        if (res.success) {
          ...
          // Ensure form is synced with DB data when entering Step 3
          if (dbUser) {
              profileForm.setValue('name', dbUser.name);
              profileForm.setValue('username', dbUser.username);
          }
        } else {
          toast.error(res.error);
        }
          ...
          <div className="w-full max-w-md space-y-8">
            ...
            <div className="text-center lg:text-left">
              ...
              <p className="mt-2 text-slate-500">
                {step === 1 && "Security Check. Verifying ownership of " + (session?.user?.email || "your email")}
            ...
            {step === 1 && (
              <div className="space-y-6 animate-in fade-in slide-in-from-right-8 duration-500">
                <div className="p-8 bg-slate-50 border border-slate-100 rounded-2xl flex flex-col items-center gap-4 text-center">
                  ...
                  <div>
                    <div className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Verifying Email</div>
                ...
                <button onClick={() => signOut()} className="w-full text-sm text-slate-400 hover:text-slate-600 font-medium flex items-center justify-center gap-2 mx-auto transition-colors">
            ...
            {step === 2 && (
              <form onSubmit={passwordForm.handleSubmit(handleVerify)} className="space-y-8 animate-in fade-in slide-in-from-right-8 duration-500">
                ...
                <div className="space-y-4">
                  ...
                  {/* FULL WIDTH SPACED OTP */}
                  <div className="w-full">
                    <Controller
                      control={passwordForm.control}
                      name="otp"
                      render={({ field }) => (
                        <InputOTP maxLength={6} {...field} containerClassName="flex w-full justify-between gap-2">
                          {/* Map slots to fill space evenly */}
